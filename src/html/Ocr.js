...
const Ocr = () => {
    ...
    const Datos = (async () => {
        ...
        const result = await axios.post('https://app.soluziona.cl/API_v1_prod/Soluziona/OCRBonos/api/OCR/QA/OCR/Bonos/Informe', { dato: "" }, { headers: { "Authorization": `Bearer ${""}` } })
                ...
                <div className="row flex-nowrap">
                    ...
                    <main className="col ps-md-2 pt-2">
                        ...
                        <div className="m-xs-0 m-lg-4"> <div className="page-header pt-3">
                            <h2 className="page-header col-sm-12 col-lg-3 mt-lg-0 mt-sm-2 text-black">Ocr</h2>
                        </div>
                            <hr />
                                        ...
                                        <table className="table">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th> nombreArchivo </th>
                                                    <th> canal </th>
                                                    <th> categoriaDocumento </th>
                                                    <th> companiaDocumento </th>
                                                    <th> documento1 </th>
                                                    <th> ocr_proceso_origen </th>
                                                    <th> camposEncontrados </th>
                                                    ...
                                                    datafull.map((data, index) => (<tr key={index}>
                                                        ...
                                                        <td> {data.rut} </td>
                                                        <td> {data.nro_denuncio} </td>
                                                        <td> {data.nombreArchivo} </td>
                                                        <td> {data.canal} </td>
                                                        <td> {data.categoriaDocumento} </td>
                                                        <td> {data.companiaDocumento} </td>
                                                        <td> {data.documento1} </td>
                                                        <td> {data.ocr_proceso_origen } </td>
                                                        <td>{data.camposEncontrados === "0" ? "no procesado" : data.camposEncontrados}</td>
